...
public class TimeRushGameManager {
    ...
    private int score;
    private int currentStreak;  // Added for streak tracking
    ...
    public void updateTimer() {
        currentTime -= 0.1;  // Update in smaller increments for smoother display
        if (currentTime <= 0) {
            currentTime = 0;
            isGameOver = true;
        }
    ...
    public void addScore(boolean isCorrect) {
        if (isCorrect) {
            // Increase streak and calculate score
            currentStreak++;
            int points = BASE_POINTS;

            // Add phase multiplier
            switch (currentPhase) {
                case 1:
